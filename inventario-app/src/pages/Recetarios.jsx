...
function parseExcelRecetarios(buffer) {
  ...
  for (const row of dataRows) {
    ...
    map.get(skuOdoo).ingredientes.push({
      nombre: ingNombre, sku: ingSku || null, producto_id: null,
      cantidad, unidad_medida: unidad, costo_unitario: costo
    })
  }
...
function TabRecetas() {
  ...
  const canWrite = canEdit('recetarios')
  const canDel   = canDeletePerm('recetarios')
  ...
  const handleFileChange = async (e) => {
    ...
    if (!file) return
    try { setPreview(parseExcelRecetarios(await file.arrayBuffer())) }
...
function ModalReceta({ receta, readOnly, onClose, onCreate, onUpdate }) {
  ...
  return (
    <div className="fixed inset-0 bg-slate-900/50 backdrop-blur-sm flex items-center justify-center z-50 p-4">
      <div className="bg-white dark:bg-slate-800 rounded-2xl shadow-xl max-w-3xl w-full max-h-[92vh] overflow-hidden flex flex-col">
        <div className="relative overflow-hidden bg-gradient-ocean p-5 shrink-0">
        ...
        <div className="p-6 overflow-y-auto flex-1 space-y-5">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div className="md:col-span-3">
              ...
              <input value={form.nombre} onChange={e => setField('nombre', e.target.value)} disabled={readOnly} placeholder="ej: Pizza Margherita"
            ...
            <div>
              ...
              <input value={form.sku_odoo} onChange={e => setField('sku_odoo', e.target.value.toUpperCase())} disabled={readOnly} placeholder="ej: PIZZA-MAR-M"
            ...
            <div>
              ...
              <input value={form.sku_template} onChange={e => setField('sku_template', e.target.value.toUpperCase())} disabled={readOnly} placeholder="ej: PIZZA-MAR"
                className="w-full px-4 py-2.5 border border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-700 text-slate-900 dark:text-slate-100 rounded-xl focus:ring-2 focus:ring-primary-500 font-mono disabled:opacity-60" />
            </div>
          </div>
          <div>
                ...
                <div className="overflow-x-auto max-h-80 overflow-y-auto">
                  <table className="w-full">
                    <thead className="bg-slate-50 dark:bg-slate-700/50 sticky top-0 z-10">
                      <tr>
                        <th className="px-3 py-2.5 text-left text-xs font-semibold text-slate-600 dark:text-slate-300 uppercase min-w-[220px]">Producto</th>
                        <th className="px-3 py-2.5 text-left text-xs font-semibold text-slate-600 dark:text-slate-300 uppercase w-20">Espec.</th>
                        <th className="px-3 py-2.5 text-left text-xs font-semibold text-slate-600 dark:text-slate-300 uppercase w-16">Unidad</th>
                        <th className="px-3 py-2.5 text-left text-xs font-semibold text-slate-600 dark:text-slate-300 uppercase w-20">Costo/u</th>
                        <th className="px-3 py-2.5 text-left text-xs font-semibold text-slate-600 dark:text-slate-300 uppercase w-24">Cantidad</th>
                        <th className="px-3 py-2.5 text-right text-xs font-semibold text-slate-600 dark:text-slate-300 uppercase w-24">Subtotal</th>
                    ...
                    <tbody className="divide-y divide-slate-100 dark:divide-slate-700 bg-white dark:bg-slate-800">
                      {form.ingredientes.map((ing, i) => (
                        <tr key={i}>
                              ...
                              {ing.producto_id && activeSearchRow !== i ? (
                                ...
                                <div>
                                  ...
                                  {activeSearchRow === i && prodSearchTerm.length > 1 && (
                                    <div className="absolute z-30 left-0 right-0 mt-1 max-h-52 overflow-y-auto bg-white dark:bg-slate-700 border border-slate-300 dark:border-slate-600 rounded-xl shadow-lg">
                          ...
                          <td className="px-3 py-2 text-xs text-slate-500">{ing.especificacion || '—'}</td>
                          <td className="px-3 py-2 text-xs text-slate-500">{ing.unidad_medida || '—'}</td>
                          <td className="px-3 py-2 text-sm text-slate-700 dark:text-slate-300">{fmtCosto(ing.costo_unitario)}</td>
                          <td className="px-3 py-2">
                            <input type="number" value={ing.cantidad || ''} onChange={e => updateCantidad(i, e.target.value)}
                              ...
                              className="w-full px-2 py-1.5 text-sm text-center font-bold border border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-700 text-slate-800 dark:text-slate-200 rounded-lg focus:ring-2 focus:ring-primary-500 disabled:opacity-60" />
                          </td>
                          <td className="px-3 py-2 text-right text-sm font-bold text-slate-900 dark:text-slate-100">{fmtCosto((ing.costo_unitario || 0) * (ing.cantidad || 0))}</td>
                          <td className="px-2 py-2 text-center">
                            {!readOnly && <button type="button" onClick={() => removeIngrediente(i)} className="p-1 text-danger-500 hover:bg-danger-50 rounded-lg"><Trash2 size={13} /></button>}
                          </td>
                        </tr>
                      ))}
                    </tbody>
          ...
          <div className="flex items-center justify-between px-5 py-3 bg-green-50 dark:bg-green-900/20 border border-green-200 dark:border-green-700 rounded-xl">
